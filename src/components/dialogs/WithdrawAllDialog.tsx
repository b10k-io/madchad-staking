...
export default function WithdrawAllDialog({ isOpen, openModal, closeModal }: IDepositDialog) {
    ...
    const { data: feeForAddress } = useContractRead(contract, "feeForAddress", address)
    const { mutateAsync: withdrawAll, status, error } = useContractWrite(contract, "withdrawAll");
    ...
    useEffect(() => {
        ...
        if (balanceOf && feeForAddress) {
            const amount = (balanceOf as BigNumber).sub(feeForAddress as BigNumber)
            setAmount(amount)
        }

    }, [balanceOf, feeForAddress])
        ...
        <DialogWrapper isOpen={isOpen} openModal={openModal} closeModal={closeModal}>
            <div className='flex flex-col gap-8'>
                ...
                {status === "idle" &&
                    <>
                        <div>
                            {feeRate && feeRate && feeForAddress && feeForAddress.gt(0) && <div className='p-3 text-sm bg-amber-100 rounded border-0 border-amber-300 text-amber-900'>One or more deposits are less than {formatDuration(feeCliff)} old and are therefore subject to a {feeRate.toNumber()}% early withdrawl fee.</div>}
                        </div>
                            ...
                            <table className='table-fixed w-full'>
                                <tbody>
                                    <tr>
                                        ...
                                        <td className={tdClass}>{balanceOf && formatCommify(balanceOf)}</td>
                                    </tr>
                                    <tr>
                                        <td className={tdClass}>Fee</td>
                                        <td className={tdClass}>{feeForAddress && formatCommify(feeForAddress)}</td>
                                    </tr>
                                    <tr>
